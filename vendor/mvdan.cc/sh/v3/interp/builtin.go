...
// BuiltinExit returns an error carrying a non-fatal exit status, for
// builtins implemented outside this package via a [CallHandlerFunc].
func BuiltinExit(code uint8) error {
	return errBuiltinExitStatus(exitStatus{code: code})
}
...
func (r *Runner) builtin(ctx context.Context, pos syntax.Pos, name string, args []string) (exit exitStatus) {
	...
	case "read":
		...
		exactN := 0
		delim := byte('\n')
		// bash accepts attached option arguments: read -n2 v, read -d:
		var readArgs []string
		for _, a := range args {
			if len(a) > 2 && a[0] == '-' && a[1] != '-' &&
				strings.ContainsAny(a[1:2], "nNdp") {
				readArgs = append(readArgs, a[:2], a[2:])
				continue
			}
			readArgs = append(readArgs, a)
		}
		fp := flagParser{remaining: readArgs}
		for fp.more() {
			switch flag := fp.flag(); flag {
			case "-s":
				silent = true
			...
			case "-n":
				val := fp.value()
				if val == "" {
					return failf(2, "read: -n: option requires an argument\n")
				}
				nChars, _ = strconv.Atoi(val)
			case "-N":
				val := fp.value()
				if val == "" {
					return failf(2, "read: -N: option requires an argument\n")
				}
				exactN, _ = strconv.Atoi(val)
			case "-d":
				val := fp.value()
				if val == "" {
					delim = '\x00'
		...
		if terminal {
			...
			line, err = r.readDelim(ctx, raw, delim, nChars, exactN)
		}
		if readArray {
	...
	case "trap":
		...
		for _, arg := range args {
			...
			default:
				sig, ok := signalByName(arg)
				if !ok {
					return failf(2, "trap: %s: invalid signal specification\n", arg)
				}
				r.setSigCallback(sig, arg, callback)
			}
		}
...
func (r *Runner) readLine(ctx context.Context, raw bool) ([]byte, error) {
	return r.readDelim(ctx, raw, '\n', 0, 0)
}

// readDelim reads one record from stdin, terminating at delim (or at
// nChars characters for read -n, or at exactly exactN characters for
// read -N). It implements the read builtin's -n/-N/-d flags.
func (r *Runner) readDelim(ctx context.Context, raw bool, delim byte, nChars, exactN int) ([]byte, error) {
	if r.stdin == nil {
	...
	for {
		if exactN > 0 && len(line) >= exactN {
			return line, nil
		}
		if nChars > 0 && len(line) >= nChars {
			return line, nil
		}
		var buf [1]byte
