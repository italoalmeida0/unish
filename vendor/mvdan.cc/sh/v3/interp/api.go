...
type Runner struct {
	...
	// extraFds holds the redirection targets of file descriptors above 2,
	// as in `exec 3>file` followed by `echo >&3`.
	extraFds   map[int]io.Writer
	extraInFds map[int]io.Reader

	// Fake signal callbacks
	callbackErr  string
	callbackExit string

	// Signal traps registered via the trap builtin; like bash, they run
	// between commands rather than re-entrantly.
	sigMu        sync.Mutex
	sigCallbacks map[os.Signal]string
