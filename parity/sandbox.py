...
CASES = [
    ...
    ("pkill -a lists ancestors form", "sleep 30 & sleep 0.3; /tmp/unish -c \"pkill -a -f 'sleep 30'\"; echo rc=$?", "rc=0\n", 0),
    ("pkill -g process group", "sleep 30 & sleep 0.3; /tmp/unish -c \"pkill -g 0 -f 'sleep 30'\"; echo rc=$?", "rc=0\n", 0),
    ("pkill -l lists signal names", "/tmp/unish -c \"pkill -l\" | grep -c TERM", "1\n", 0),
    ("pkill -n newest only", "sleep 30 & sleep 0.3; /tmp/unish -c \"pkill -n -f 'sleep 30'\"; echo rc=$?", "rc=0\n", 0),
