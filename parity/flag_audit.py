...
FIXTURES = {
    ...
    # --- file-tree commands: a directory to act on ---
    "cp":      {"files": {"a.txt": "a", "b.txt": "b"}, "base": "a.txt copy.txt"},
    "mv":      {"files": {"a.txt": "a"}, "base": "a.txt moved.txt"},
    "rm":      {"files": {"a.txt": "a"}, "base": "a.txt"},
    "ln":      {"files": {"a.txt": "a"}, "base": "a.txt link.txt"},
    "mkdir":   {"base": "newdir"},
    ...
    "stat":    {"files": {"a.txt": "a"}, "base": "a.txt"},
    "touch":   {"files": {"a.txt": "a"}, "base": "a.txt"},
    "chmod":   {"files": {"a.txt": "a"}, "base": "a.txt"},
    "du":      {"files": {"a.txt": "a"}, "base": "."},
    "find":    {"files": {"a.txt": "a"}, "base": "."},
    "tar":     {"files": {"a.txt": "a", "b.txt": "b"}, "base": "a.txt b.txt"},
    "gzip":    {"files": {"a.txt": "hello\n"}, "base": "a.txt"},
...
# silently skipped).
NEEDS_FIXTURE = (
    "--tty", "--interactive", "--follow", "--net", "--listen",
    "--devices", "--context", "--dereference", "--watch", "--persistent",
)
